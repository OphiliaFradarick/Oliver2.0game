...
trollimg = loadAnimation("Images/troll1.png","Images/troll2.png","Images/troll3.png")

//obstaclecouresimage=loadAnimation("Images/obstacle coures 1.jpg","Images/obstacle coures 2.jpg","Images/obstacle coures 3.jpg")
...
if(trollsgroup.isTouching(ninja) || dragongroup.isTouching(ninja)){
  trollsgroup[0].destroy;
 trollattack = trollattack -1

 if(trollattack === 1250){
  healthbar.addImage("health",healthbarimage2);
  healthbar.scle = 0.5
 }
 else if(trollattack === 700){
  healthbar.addImage("health",healthbarimage3);
 }
 else if(trollattack === 300){
  healthbar.addImage("health",healthbarimage4);
 }
...
stroke("white");
fill ("yellow");
text(" Life Time : " + trollattack, 600,50)
}

function spawntrolls(){
  if(frameCount % 80 === 0){

    var trolls = createSprite(1500,450,10,70);
    trolls.addAnimation("trolls",trollimg);
    trolls.scale= 0.8;
    trolls.velocityX=-6;
    trolls.lifetime = 300;
